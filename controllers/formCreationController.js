...
const correoANombre = {
    ...
    "operacionescomerciales@merkahorrosas.com": "Andrés Gómez (Coordinador Punto de Ventas)",
    "compras@merkahorrosas.com": "Julián Hurtado (Coordinador Estratégico de Compras)",
...
const approvalFlow = {
    merkahorro: ['area', 'director', 'gerencia', 'calidad', 'seguridad'],
    construahorro: ['director', 'gerencia', 'calidad', 'seguridad'],
...
const createEmailData = (body, data) => {
    ...
    return {
        ...data,
        ...parsedData,
        poblacion_focalizada: parsedData.poblacionFocalizada,
...
export const crearFormulario = async (req, res) => {
    try {
        ...
        const requiredFields = {
            ...
            competenciasCulturales, competenciasCargo, responsabilidades, calidad, seguridad
        };

        if (!isConstruahorroForm && !area) {
            return res.status(400).json({ error: 'El campo área es obligatorio para Merkahorro y Megamayoristas' });
        }
...
export const decision = async (req, res) => {
    try {
        ...
        const company = solicitud.isConstruahorro ? 'construahorro' : (solicitud.isMegamayoristas ? 'megamayoristas' : 'merkahorro');
        const flow = approvalFlow[company];
        const roleIndex = flow.indexOf(role);
        const previousRole = flow[roleIndex - 1];
        
        const expectedStatus = previousRole ? `pendiente por ${previousRole}` : 'pendiente por area';
        
        if (solicitud.estado !== expectedStatus && solicitud.estado !== `aprobado por ${previousRole}`) {
            return res.status(400).json({ error: `Estado no válido para aprobación/rechazo por ${role}` });
        }
        ...
        if (decision === 'rechazar') {
            ...
            const creatorEmail = solicitud.isConstruahorro ? solicitud.director : solicitud.area;
...
export const obtenerHistorial = async (req, res) => {
    try {
        ...
        const formattedHistorial = historial.map(item => {
            ...
            const approvalSteps = isConstruahorro || isMegamayoristas
                ? ['director', 'gerencia', 'calidad', 'seguridad']
                : ['area', 'director', 'gerencia', 'calidad', 'seguridad'];
